        ...
        <div class="timeaxis-body">
            <div class="timeaxis-scroll-x">
                <div class="timeaxis-item" :class="{active:currentStep>index}" v-for="(item,index) in timeList" :key="index">
                    <div class="item-con" @click="setTime">
                        <div class="time-con">
...
import { setTimeAxis,getTimeAxis } from '@/api/timeaxis/timeaxis'
...
export default {
    ...
    methods: {
        ...
        setTimeAxis(){
            var date = dealDatetime(new Date())
            var timeType = this.currentStep + 1;
            var params = {
            ...
            setTimeAxis(params).then((res)=>{
                console.log('时间轴数据返回结果：',res);
                if(res.data.code==200){
                    this.currentStep = timeType;
                    this.getTimeAxis();
                }
            })
        },
        getTimeAxis(){
            ...
            getTimeAxis(params).then((res)=>{
                console.log('获取时间轴',res);
                if(res.data.code==200){
                    this.timeList = []
                    var lists = res.data.data
                    lists.forEach((item,index)=>{
                        var value = item.value;
                        var date = (value && value.createTime?(value.createTime).substring(0,10):'-')
                        var time = (value && value.createTime?(value.createTime).substring(11,19):'-')
                        this.timeList.push(
                            {
                                date: date,
                                time: time,
